...
void ntupleClass_muonid::Loop(TString type, TString datasetName){
    ...
    if(datasetName.Contains("2017Ds") || datasetName.Contains("2017B0") || datasetName.Contains("2017Bp")) isT3M = true;
    ...
    TString weights_folder = "";
    if(datasetName.Contains("2018"))
        weights_folder = "/lustrehome/fsimone/MuonID_study/MuonMVA_2018/Phase_space_reweighing_tools/";
    if(datasetName.Contains("2017"))
        weights_folder = "/lustrehome/fsimone/MuonID_study/MuonMVA_2017/Phase_space_reweighing_tools/";

    TFile *f_weights_barrel = TFile::Open(weights_folder+"/PT_reweighting_muonid_barrel.root", "read");
    TH2D *h_weights_barrel = (TH2D*)f_weights_barrel->Get("hweight_pteta");
    if (isVerbose) cout<<"opened pt-eta weights file: "+weights_folder+"/PT_reweighting_muonid_barrel.root"<<endl;
    TFile *f_weights_endcap = TFile::Open(weights_folder+"/PT_reweighting_muonid_endcap.root", "read");
    TH2D *h_weights_endcap = (TH2D*)f_weights_endcap->Get("hweight_pteta");
    if (isVerbose) cout<<"opened pt-eta weights file: "+weights_folder+"/PT_reweighting_muonid_endcap.root"<<endl;
    ...
    for (Long64_t jentry=0; jentry<nentries; jentry++) {
        ...
        for(int h=0; h<Trigger_hltname->size(); h++) {
            ...
            //HLT 2018
            if( datasetName.Contains("2018") && strncmp(hltName, "HLT_DoubleMu3_TkMu_DsTau3Mu_v", 29) == 0 && Trigger_hltdecision->at(h) == 1) {
                hlt_fired = 1;
            }
            //HLT 2017
            if( datasetName.Contains("2017") && strncmp(hltName, "HLT_DoubleMu3_Trk_Tau3mu_v", 26) == 0    && Trigger_hltdecision->at(h) == 1) {
                hlt_fired = 1;
            }
        }
