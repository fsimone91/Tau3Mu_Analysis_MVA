#ifndef PLOT_ROC_TEST_TRAIN_H_
#define PLOT_ROC_TEST_TRAIN_H_
...
{
    TString file_name = workdir+TMVA_inputpath+"outputTree_minbias.root";
    TH1F *h_test_signal;
    TH1F *h_test_bkg;
    TH1F *h_test_signal2;
    TH1F *h_test_bkg2;
    ...
    if (categ=="B") cat = "1";
    if (categ=="C") cat = "2";
    TString signal = "weight*(isMC>0 && isMC<4 && category =="+cat+")";
...
TGraph* plot_ROC(TString categ, TH1F *h_signal, TH1F *h_bkg){
    ...
    gr->SetTitle("2018 "+categ+" - ROC;Signal Efficiency;Background rejection");
    return gr;
}
...
void plot_wp_ROC(TString categ, TH1F *h_test_signal, TH1F *h_test_bkg, BDTcut3d BDTcutvalues){
 
     //BDTcut3d BDTcutvalues = Get_BDT_cut_3D(categ);

     gStyle->SetOptTitle(1);
     TGraph* ROC =  plot_ROC(categ, h_test_signal, h_test_bkg);
     TCanvas *c1 = new TCanvas("c1","ROC "+categ,150,10,800,800);
 //    ROC->GetYaxis()->SetRangeUser(0.5,1.05);
     ...
     l->DrawLatex(x[0],y[0]+0.015,Form("c = %4.3f",BDTcutvalues.c));
     l->DrawLatex(x[0],y[0]-0.025,Form("(%4.2f, %4.2f)",x[0], y[0]));
     l->DrawLatex(x[1],y[1]+0.015,Form("b = %4.3f",BDTcutvalues.b));
     l->DrawLatex(x[1],y[1]-0.025,Form("(%4.2f, %4.2f)",x[1], y[1]));
     l->DrawLatex(x[2],y[2]+0.015,Form("a = %4.3f",BDTcutvalues.a));
     l->DrawLatex(x[2],y[2]-0.025,Form("(%4.2f, %4.2f)",x[2], y[2]));
     gr->Draw("same *p");
