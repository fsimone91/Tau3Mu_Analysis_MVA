...
void BDT_optimal_cut_v3(TString year) 
{
    ...
    for(int k=0; k<ncat; k++)
    {
        ...
        TString file_name = TMVA_inputpath+"_"+year+"_outputTree.root";
        ...
        TString signal = "weight*(isMC>0 && isMC<4 && category =="+c+")";
        ...
        BDTcut3d cut_value = Get_BDT_cut_3D(cat_label[k], file_name);
        //Draw BDT working point on ROC curve
        plot_wp_ROC(cat_label[k], h_test_signal, h_test_bkg, cut_value);
        ...
        leg2->AddEntry(h_test_signal2,"2018 "+cat_label[k]+" - signal","f");
        leg2->AddEntry(h_test_bkg2,   "2018 "+cat_label[k]+" - bkg","f");
        leg2->Draw();
        c2->Update();
