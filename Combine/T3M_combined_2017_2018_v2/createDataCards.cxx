...
void 
createDataCards(TString inputfile, string year, int signalsample = 0, Bool_t dobands = false)
{
  ...
  cat_names.push_back("A1_"+year);
  cat_names.push_back("B1_"+year);
  cat_names.push_back("C1_"+year);
  cat_names.push_back("A2_"+year);
  cat_names.push_back("B2_"+year);
  cat_names.push_back("C2_"+year);
  ...
  TString filename("temp_workspace_"+year+".root");
  ...
  for(unsigned int category=0; category< NCAT; category++)
    {
      ...
      RooCBShape CB_final(TString::Format("CB_final_%s",cat_names.at(category).c_str()),"CB PDF",*m3m,mean,sigma_cb,*sig_alpha,*sig_n) ;
      RooGaussian GS_final(TString::Format("GS_final_%s",cat_names.at(category).c_str()),"GS PDF",*m3m,mean,*sig_gaus_sigma) ;
      RooAddPdf signal(TString::Format("SignalModel_%s",cat_names.at(category).c_str()),"",RooArgList(CB_final,GS_final), *CBFraction);
      //fitting again with fixed mean and width
      signal.fitTo(*sigToFit[category],Range("signal"), SumW2Error(kTRUE));
      ...
      w->defineSet(TString::Format("SigPdfParam_%s",cat_names.at(category).c_str()), RooArgSet(
                       ...
                       *sig_gaus_sigma, //*w->var("sig_gaus_sigma"+TString::Format("_%s",cat_names.at(category).c_str())),
                       *sig_alpha, //*w->var("sig_alpha"+TString::Format("_%s",cat_names.at(category).c_str())),
                       *sig_n)); //*w->var("sig_n"+TString::Format("_%s",cat_names.at(category).c_str()))));
...
AddSigData(TString file, RooWorkspace* w, std::vector<string> cat_names) {
  ...
  for(unsigned int category=0; category< NCAT; category++){
    std::string cat_histo = cat_names.at(category).c_str();
    cat_histo = cat_histo.substr(0,2); //drop _year
    TString name = TString::Format("Sig_%s",cat_names.at(category).c_str());
    taumass[category] = (TH1F*)f->Get(TString::Format("signal%s",cat_histo.c_str()));
  ...
  for(unsigned int category=0; category< NCAT; category++)
    {
      ...
      taumass[category] = (TH1F*)f->Get(TString::Format("background%s",cat_histo.c_str())); 
...
MakeSigWS(RooWorkspace* w, const char* fileBaseName,  std::vector<string> cat_names) {
  ...
  if(cat_names.at(0).find("2017") != std::string::npos)
      wsDir   = "workspaces_2017/";
  if(cat_names.at(0).find("2018") != std::string::npos)
      wsDir   = "workspaces_2018/";
...
void MakeDataCard(RooWorkspace* w, const char* fileBaseName, const char* fileBkgName,  std::vector<string> cat_names) {
  ...
  for (int c = 0; c < ncat; ++c) {
    ...
    if(cat_names.at(c).find("2017") != std::string::npos){
        //outFile << "lumi_13TeV_2017       lnN  1.027      - " << endl;
        //outFile << "lumi_13TeV_2017        lnN  1.027     - " << endl;
        //outFile << "DsNorm_13TeV_2017      lnN  1.033     - " << endl;
        outFile << "DsNorm_13TeV_2017      lnN  1.06      - " << endl; //2017
        outFile << "BRDToTau_13TeV_2017    lnN  1.03      - " << endl;
        outFile << "BRDsPhiPi_13TeV_2017   lnN  1.08      - " << endl;
        outFile << "BRBtoD_13TeV_2017      lnN  1.05      - " << endl;
        outFile << "BRBtoTau_13TeV_2017    lnN  1.03      - " << endl;    
        outFile << "fUnc_13TeV_2017        lnN  1.03      - " << endl;
        outFile << "DpmScaling_13TeV_2017  lnN  1.03      - " << endl;
        outFile << "BsScaling_13TeV_2017   lnN  1.04      - " << endl;
        outFile << "UncTrigger_13TeV_2017  lnN  1.14      - " << endl; //2017
        outFile << "UncBDTCut_13TeV_2017   lnN  1.05      - " << endl;
        outFile << "UncRatioAcc_13TeV_2017 lnN  1.01      - " << endl;
        outFile << "UncMuonEff_13TeV_2017  lnN  1.015     - " << endl;
    //    outFile << "MuES_13TeV_2017        lnN  1.007     - " << endl;
    //    outFile << "MuRes_13TeV_2017       lnN  1.025     - " << endl;
    }
    else if(cat_names.at(c).find("2018") != std::string::npos){
        //outFile << "lumi_13TeV_2018       lnN  1.027      - " << endl;
        //outFile << "lumi_13TeV_2018        lnN  1.027     - " << endl;
        //outFile << "DsNorm_13TeV_2018      lnN  1.033     - " << endl;
        outFile << "DsNorm_13TeV_2018      lnN  1.03      - " << endl; //2018 updated on 22 march
        outFile << "BRDToTau_13TeV_2018    lnN  1.03      - " << endl;
        outFile << "BRDsPhiPi_13TeV_2018   lnN  1.08      - " << endl;
        outFile << "BRBtoD_13TeV_2018      lnN  1.05      - " << endl;
        outFile << "BRBtoTau_13TeV_2018    lnN  1.03      - " << endl;    
        outFile << "fUnc_13TeV_2018        lnN  1.03      - " << endl;
        outFile << "DpmScaling_13TeV_2018  lnN  1.03      - " << endl;
        outFile << "BsScaling_13TeV_2018   lnN  1.04      - " << endl;
        outFile << "UncTrigger_13TeV_2018  lnN  1.05      - " << endl; //2018 updated on 22 march
        outFile << "UncBDTCut_13TeV_2018   lnN  1.05      - " << endl;
        outFile << "UncRatioAcc_13TeV_2018 lnN  1.01      - " << endl;
        outFile << "UncMuonEff_13TeV_2018  lnN  1.015     - " << endl;
    //    outFile << "MuES_13TeV_2018        lnN  1.007     - " << endl;
    //    outFile << "MuRes_13TeV_2018       lnN  1.025     - " << endl;
    }
