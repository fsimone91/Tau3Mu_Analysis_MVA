...
void Ds_syst() 
{
    ...
    std::vector<TString> era = {'A', 'B','C','D'};
    ...
    for (Int_t i=0;i<nrun;i++) {
      ...
      hDs->Fill(i,Dsyield[i]/Lumi_data_control[i]);
      hDs->GetXaxis()->SetBinLabel(i+1, era[i]);
      Double_t err = pow(pow(Dsyield_err[i]/Lumi_data_control[i],2) + pow(0.023*Lumi_data_control[i]*Dsyield[i]/pow(Lumi_data_control[i],2),2),0.5);
      hDs->SetBinError(i+1, err);

      cout<<"MC "<<Lumi_data_control[i]*xsection_mc*BR/N_MC * Dsyield_MC<<endl;
      ...
      Ds_tot+=Dsyield[i];
      Ds_tot_MC+=Lumi_data_control[i]*xsection_mc*BR/N_MC * Dsyield_MC;
    }
    ...
    hDs->GetXaxis()->SetTitle("2018 era");
    ...
    h3mu_ratio->GetXaxis()->SetTitle("2018 era");
    ...
    hDs_ratio->GetXaxis()->SetTitle("2018 era");
    ...
    h3mu_or->GetXaxis()->SetTitle("2018 era");
