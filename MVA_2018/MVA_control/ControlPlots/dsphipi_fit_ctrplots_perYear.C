...
void dsphipi_fit_ctrplots_perYear() 
{
    ...
    TFile *fout = new TFile("dsphipi_all_output_"+TMVA_inputpath_control+"new_perYear.root", "RECREATE");
    fout->cd();
    ...
    const int nrun = sizeof(inputpath_datarun_control)/sizeof(inputpath_datarun_control[0]);
    ...
    TString common_cut = 
                         ...
                         //"(abs(phiMass-1.02)<0.020) &&" //for isolation vars
    ...
    tdata_all->Draw("tripletMass>>h_tripletmass_full"+binning_mass, invmass_all);
    ...
    for(int i = 0; i<nrun; i++){
        ...
        c5->SaveAs("dsphipi_fit_perYear_new"+category+".png");
    ...
    c5->SaveAs("dsphipi_fit_perYear_new_full.png");
...
//                         "MuonIDeval_Mu2.MuonID",
  ...
  //                   "MVAmuID_glb_mu2",
    ...
    for(int k = 0; k<n; k++){
        ...
        leg->AddEntry(hdata_sgn_plus[k],"data "+run_lable[nrun-1]+" (SB +10\% subt.)","f");
        leg->AddEntry(hdata_sgn_minus[k],"data "+run_lable[nrun-1]+" (SB -10\% subt.)","f");
        leg->Draw();
        ...
        h_x_ratio->Write();
        c2->SaveAs("plots_AN/"+varname+"_"+TMVA_inputpath_control+"_"+run_lable[nrun-1]+".png");
        fout->cd();
