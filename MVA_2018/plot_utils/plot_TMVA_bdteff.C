...
void plot_TMVA_bdteff(TString category) 
{
    ...
    binning_bkg = "(76, 1.62, 2.0)";
    binning_sig = "(32, 1.69, 1.85)";
    ...
    for(int i=0; i<n_cut; i++){
       ...
       hratio_sig[i]->GetXaxis()->SetTitle("m(3#mu) (GeV)");
    ...
    hden_sig_plot->GetXaxis()->SetTitle("m(3#mu) (GeV)");
    ...
    leg_signal->AddEntry(hden_sig_plot, sample+" m(3#mu) (GeV)", "f");
   ...
   c_sig->SaveAs(workdir+TMVA_inputpath+category+"/"+TMVA_inputpath+category+"_"+varname+"bdteff_signal"+sample+".png");
    ...
    for(int i=0; i<n_cut; i++){
       ...
       hratio_bkg[i]->GetXaxis()->SetTitle("m(3#mu) (GeV)");
    ...
    hden_bkg_plot->GetXaxis()->SetTitle("m(3#mu) (GeV)");
    ...
    leg_bkg->AddEntry(hden_bkg_plot, "Data m(3#mu) (GeV)", "f");
    for(int i=0; i<n_cut; i++){
