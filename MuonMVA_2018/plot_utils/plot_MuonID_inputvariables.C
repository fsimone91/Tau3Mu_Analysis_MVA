...
void plot_MuonID_inputvariables() 
{
    ...
    TFile *fout = new TFile("../plots/"+TMVA_MuonID_inputpath+".root", "RECREATE");
    fout->cd();
...
*/
    int n = var_MuonID_train_def.size();
    TString var[n];
    for(int i=0; i<n; i++) var[i] = var_MuonID_train_def[i];
    ...
    for(int i = 0; i<n; i++){
        ...
        binning = "";
        if(varname.Contains("mu_combinedQuality_chi2LocalMomentum")) binning = "(120,0,255)";
        if(varname.Contains("mu_GLnormChi2")) binning = "(110,0,55)";
        if(varname.Contains("mu_emEt03")) binning = "(100,0,300)";
        if(varname.Contains("mu_hadEt03")) binning = "(110,0,350)";
        if(varname.Contains("mu_nJets03")) binning = "(5,0,5)";
        if(varname.Contains("mu_nTracks03")) binning = "(40,0,40)";
        if(varname.Contains("mu_sumPt03")) binning = "(150,0,1000)";
        if(varname.Contains("mu_emVetoEt03")) binning = "(50,0,200)";
        if(varname.Contains("mu_hadVetoEt03")) binning = "(50,0,200)";
        if(varname.Contains("mu_trackerVetoPt03")) binning = "(60,0,300)";
        if(varname.Contains("mu_innerTrack_validFraction")) binning = "(80,0,1)";
        if(varname.Contains("mu_combinedQuality_staRelChi2")) binning = "(100,0,50)";
        ...
        TLegend*leg = new TLegend(0.35,0.7,0.75,0.9);
