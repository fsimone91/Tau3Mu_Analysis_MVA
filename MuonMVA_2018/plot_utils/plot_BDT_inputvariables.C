...
void plot_BDT_inputvariables() 
{
    ...
    for(auto i=0; i<nrun; i++){
        tdata->Add(inputpath_MuonID_datarun[i]);
        std::cout<<"Opened input file: "<<inputpath_MuonID_datarun[i]<<std::endl;
    }
    //MC 
    TChain *tmc = new TChain("FinalTree");
    tmc->Add(inputpath_MuonID_Ds);
    tmc->Add(inputpath_MuonID_B0);
    tmc->Add(inputpath_MuonID_Bp);
    ...
    for(int i = 0; i<n; i++){
        ...
        c1->SetLogy();
        c1->Update();
        c1->SaveAs("plots/"+TMVA_MuonID_inputpath+"_Data_"+varname+".png");
        ...
        c2->SetLogy();
        c2->Update();
        c2->SaveAs("plots/"+TMVA_MuonID_inputpath+"_MC_"+varname+".png");
