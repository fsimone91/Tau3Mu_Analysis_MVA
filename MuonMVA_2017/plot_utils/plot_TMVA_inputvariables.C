...
//root -l -b plot_TMVA_inputvariables.C\(\"barrel\"\,True\)
void plot_TMVA_inputvariables(TString category, bool apply_weights) 
{
    TString TMVA_filename = "../TMVA_"+TMVA_MuonID_inputpath+category;
    ...
    TFile *f_tmva = new TFile(TMVA_filename+".root","READ");
    cout<<"Opened TMVA file"<<endl;
    TTree *tTrain = (TTree*)f_tmva->Get(TMVA_MuonID_inputpath+category+"/TrainTree");
   ...
   for(int i = 0; i<n; i++){
        ...
        //bottom centre
        if(varname=="mu_eta"|| varname=="mu_phi"){
            x1=0.3; x2=0.75;
            y1=0.1; y2=0.3;
        }
