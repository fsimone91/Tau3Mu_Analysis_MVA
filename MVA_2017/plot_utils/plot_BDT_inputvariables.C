...
void plot_BDT_inputvariables() 
{
    ...
    for(int i = 0; i<n; i++){
        ...
        binning = "";
        if(varname=="Pmu3") binning = "(150,0,50)";
        ...
        if(varname=="fv_dphi3D") binning = "(50,-0.1,0.5)";
        if(varname=="fv_d3Dsig") binning = "(80,-0.1,200)";
        if(varname=="d0sig") binning = "(100,-0.1,40)";
        if(varname=="mindca_iso") binning = "(150,-0.1,2)";
        if(varname=="trkRel") binning = "(150,-0.5,50)";
        if(varname=="nMatchesMu3") binning = "(20,0,10)";
        if(varname=="fv_d3D") binning = "(40,0,10)";
        if(varname.Contains("mu_SoftMVA")) binning = "(200,-1,1)";
        if(varname.Contains("RefV")) binning = "(80,-0.02,0.04)";
        if(varname.Contains("SVx") || varname.Contains("SVy") || varname.Contains("SVz")) binning = "(80,-0.02,0.04)";
        ...
        leg->AddEntry(hdata[0],"2017 "+varname+" data_A","f");
        leg->AddEntry(hdata[1],"2017 "+varname+" data_B","f");
        leg->AddEntry(hdata[2],"2017 "+varname+" data_C","f");
        leg->Draw();
        ...
        leg2->AddEntry(hmc[0],"2017 "+varname+" mc_A","f");
        leg2->AddEntry(hmc[1],"2017 "+varname+" mc_B","f");
        leg2->AddEntry(hmc[2],"2017 "+varname+" mc_C","f");
        leg2->Draw();
        ...
        Double_t mc_ymax = hmc_merged->GetMaximum(); 
        Double_t data_ymax = hdata_merged->GetMaximum();
        Double_t ymax = std::max(mc_ymax, data_ymax);

        hmc_merged->GetYaxis()->SetRangeUser(0, ymax*1.01);

        c3->Update();
        hmc_merged->Draw("hist");
        hdata_merged->Draw("hist same");

        TLegend*leg3 = new TLegend(0.5,0.65,0.9,0.9);
        leg3->AddEntry(hmc_merged,TMVA_inputpath+"_"+varname+" mc","f");
        leg3->AddEntry(hdata_merged,TMVA_inputpath+"_"+varname+" data","f");
        leg3->Draw();
