...
void dsphipi_fit_ctrplots(TString n_sigma, ofstream& fout_ratio) 
{
    bool doAllRuns = false;
    bool savePlot = false;
    cout<<"n_sigma = "<<n_sigma<<endl;

    //open root files where to store all plots!!
    //TFile *fout = new TFile("dsphipi_all_output_displ"+n_sigma+".root", "RECREATE");
    //fout->cd();
    //open text file where to write Ds yields
    ofstream fout_yield("dsphipi_yield.txt");

    TFile *f_mc = new TFile(inputpath_DsPhiPi, "READ"); //cut8 is w/ inv mass in 1.93 - 2.01
    ...
    for(int i=0; i<nrun; i++){
        ...
        TString invmass_SB   = "puFactor*(bs_sv_d2Dsig>"+n_sigma+" && tripletMass<1.80 && tripletMass>1.70)";
        TString invmass_peak = "puFactor*(bs_sv_d2Dsig>"+n_sigma+" && tripletMass<2.01 && tripletMass>1.93)";
        TString invmass_full = "puFactor*(bs_sv_d2Dsig>"+n_sigma+" && tripletMass<2.01 && tripletMass>1.60)";
        TString binning_mass = "(42, 1.60, 2.01)";
        ...
        if(doAllRuns){
            run_lable = "2017";
            tdata_all->Draw("tripletMass>>h_tripletmass"+binning_mass, invmass_full);
            ...
            tdata->Draw("tripletMass>>h_tripletmass"+binning_mass, invmass_full);
        ...
        Double_t mass1_low = 1.84;
        //if(i==0) mass1_low = 1.828; 
        //else  mass1_low = 1.84; 
        ...
        RooRealVar nbkg("nbkg","#background events",400000,100,10000000);
  ...
  //      model.paramOn(frame,Layout(0.12, 0.4, 0.9));
        ...
        fout_yield<<nsigevents<<"\t"<<nsig_err<<"\n";
        ...
        text->Draw("same");
        c5->Update();
        if(savePlot) c5->SaveAs("plots_displacement/DsPhiPi_invmass_"+run_lable+"_displ"+n_sigma+"_26nov.png");
...
//            continue;
            ...
            if(varname=="fv_d3D") binning = "(50,0,2)";
            if(varname=="fv_d3D/fv_d3Dsig") binning = "(50,0,0.12)";
            if(varname=="bs_sv_d2Dsig") binning = "(80,-0.1,150)";
            if(varname=="bs_sv_d2D") binning = "(50,0,5)";
            if(varname=="bs_sv_d2D/bs_sv_d2Dsig") binning = "(50,0,0.06)";
            ...
            //plot makeup
            //
            hmc_sgn[k]->SetTitle(varname+" BS_SV_sig_above_"+n_sigma);
            ...
            //fout_ratio<<varname<<"\t"<<mean<<"\t"<<std_dev<<"\t"<<n_sigma<<"\n";
            //KolmogorovTest
            Double_t ks = hdata_sgn[k]->KolmogorovTest(hmc_sgn[k]);
            cout<<"+++++++++ "<<var[k]+run_lable<<" KS test: "<<ks<<endl;
            fout_ratio<<run_lable<<"\t"<<varname<<"\t"<<mean<<"\t"<<std_dev<<"\t"<<n_sigma<<"\t"<<ks<<"\n";
            ...
            if(savePlot) c2->SaveAs("plots_displacement/"+varname+"_"+run_lable+"_displ"+n_sigma+"_26nov.png");
            //if(savePlot) c2->SaveAs("plots_displacement/"+varname+"_"+run_lable+".pdf");
...
void draw_TGraph_fromFile(TString input, TString variable, TString run){

    vector<Double_t> ratio;
    vector<Double_t> ratio_err;
    vector<Double_t> sigma_cut;
    vector<Double_t> sigma_cut_err;
    vector<Double_t> KS;
    vector<Double_t> KS_err;

    vector<Double_t> ratio_avg;
    vector<Double_t> ratio_avg_err;
    vector<Double_t> sigma_cut_avg;
    vector<Double_t> sigma_cut_avg_err;
    vector<Double_t> KS_avg;
    vector<Double_t> KS_avg_err;
    ...
    if(!filein.is_open()) cout<<"FILE DOES NOT EXIST"<<endl;
    else cout<<"FILE "<<input<<" OPENED"<<endl;

    while ((filein >> temp) && (filein >> temp0) && (filein >> temp1) && (filein >> temp2) && (filein >> temp3) && (filein >> temp4))
    {
        if(temp0 == variable && temp==run){
            ratio.push_back(temp1);
            ratio_err.push_back(temp2);
            sigma_cut.push_back(temp3);
            sigma_cut_err.push_back(0.0);
            KS.push_back(temp4);
            KS_err.push_back(0.0);
            cout<<line<<"\t"<<temp1<<"\t"<<temp2<<"\t"<<temp3<<"\t"<<temp4<<endl;
            line++;

            //Each group of 4 elements are averaged
            if( (line)%4==0){
                Double_t sigma_cut_avg_val = ( sigma_cut[line-4]+sigma_cut[line-1]+sigma_cut[line-2]+sigma_cut[line-3] )/4.0;
                Double_t ratio_avg_val = ( ratio[line-4]+ratio[line-1]+ratio[line-2]+ratio[line-3] )/4.0;
                Double_t ratio_avg_err_val = sqrt( pow(ratio_err[line-4],2) + pow(ratio_err[line-1],2) + pow(ratio_err[line-2],2) + pow(ratio_err[line-3],2)  );
                Double_t KS_avg_val = ( KS[line-4]+KS[line-1]+KS[line-2]+KS[line-3] )/4.0;

                cout<<"    line="<<line<<" sigma_cut_avg_val = "<<sigma_cut_avg_val<<" ratio_avg_val = "<<ratio_avg_val<<" pm ratio_avg_err_val = "<<ratio_avg_err_val<<" KS_avg_val = "<<KS_avg_val<<endl;
                sigma_cut_avg.push_back( sigma_cut_avg_val );
                sigma_cut_avg_err.push_back( 0.0 );
                ratio_avg.push_back( ratio_avg_val );
                ratio_avg_err.push_back( ratio_avg_err_val );
                KS_avg.push_back( KS_avg_val );
                KS_avg_err.push_back( 0.0 );
                line_avg++;
            }
        }
    }
    cout<<"line "<<line<<" line_avg "<<line_avg<<endl;
    TGraphErrors *ge = new TGraphErrors(line, &(sigma_cut[0]), &(ratio[0]), &(sigma_cut_err[0]), &(ratio_err[0]) );
    TCanvas *c = new TCanvas("c","c",150,10,990,660);
    ge->Draw("apel");
    ge->GetXaxis()->SetTitle("sigma_cut");
    ge->GetYaxis()->SetTitle("Data/MC ratio "+variable+" - "+run);
    c->Update();
    c->SaveAs(variable+"_"+run+"_ratio_2dec.png");

    TGraphErrors *ge2 = new TGraphErrors(line, &(sigma_cut[0]), &(KS[0]), &(sigma_cut_err[0]), &(KS_err[0]) );
    TCanvas *c2 = new TCanvas("c2","c2",150,10,990,660);
    ge2->Draw("apel");
    ge2->GetXaxis()->SetTitle("sigma_cut");
    ge2->GetYaxis()->SetTitle("KS test "+variable+" - "+run);
    c2->Update();
    c2->SaveAs(variable+"_"+run+"_KS_2dec.png");

    TGraphErrors *ge_avg = new TGraphErrors(line_avg, &(sigma_cut_avg[0]), &(ratio_avg[0]), &(sigma_cut_avg_err[0]), &(ratio_avg_err[0]) );
    TCanvas *c_avg = new TCanvas("c","c",150,10,990,660);
    ge_avg->Draw("apel");
    ge_avg->GetXaxis()->SetTitle("sigma_cut");
    ge_avg->GetYaxis()->SetTitle("Data/MC ratio "+variable+" - "+run);
    c_avg->Update();
    c_avg->SaveAs(variable+"_avg_"+run+"_ratio_2dec.png");

    TGraphErrors *ge2_avg = new TGraphErrors(line_avg, &(sigma_cut_avg[0]), &(KS_avg[0]), &(sigma_cut_avg_err[0]), &(KS_avg_err[0]) );
    TCanvas *c2_avg = new TCanvas("c2","c2",150,10,990,660);
    ge2_avg->Draw("apel");
    ge2_avg->GetXaxis()->SetTitle("sigma_cut");
    ge2_avg->GetYaxis()->SetTitle("KS test "+variable+" - "+run);
    c2_avg->Update();
    c2_avg->SaveAs(variable+"_avg_"+run+"_KS_2dec.png");
}

void dsphipi_fit_ctrplots_displacement_cut_scan(){
    //open text file where to write average of ratio plot
    ofstream fout_ratio("dsphipi_KS_3dec.txt");
    fout_ratio<<"run\tvarname\tmean\tstd_dev\tn_sigma\tKS\n";

    int N = 700;
    double nsigma = 1;
    for(int i=0;i<N;i++){
        nsigma=1+i*0.01;
        dsphipi_fit_ctrplots(to_string(nsigma), fout_ratio);
    }
    fout_ratio.close();
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3D", "2017B");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3D", "2017C");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3D", "2017D");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3D", "2017E");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3D", "2017F");

    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3Dsig", "2017B");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3Dsig", "2017C");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3Dsig", "2017D");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3Dsig", "2017E");
    draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS_3dec.txt", "fv_d3Dsig", "2017F");

    //draw_TGraph_fromFile("/lustrehome/fsimone/MVA_2017/MVA_control/ControlPlots/dsphipi_KS.txt", "fv_d3Dsig");
        
//    dsphipi_fit_ctrplots("0", fout_ratio);
//    dsphipi_fit_ctrplots("1", fout_ratio);
//    dsphipi_fit_ctrplots("2", fout_ratio);
//    dsphipi_fit_ctrplots("3", fout_ratio);
//    dsphipi_fit_ctrplots("4", fout_ratio);
//    dsphipi_fit_ctrplots("4", fout_ratio);
//    dsphipi_fit_ctrplots("5", fout_ratio);
//    dsphipi_fit_ctrplots("6", fout_ratio);
//    dsphipi_fit_ctrplots("7", fout_ratio);
//    dsphipi_fit_ctrplots("8", fout_ratio);
}
