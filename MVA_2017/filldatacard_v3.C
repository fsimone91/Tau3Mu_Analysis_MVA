...
void filldatacard_v3() 
{
    ...
    for(auto i = 0; i<3; i++){
        ...
        TH1F * hSignal3        ;// = new TH1F ("signal"+category+"3","Triplet mass "+category+"2",42, 1.600000, 2.020000);        
        ...
        ttree->Draw("tripletMass>>hTriplMass_data3"+binning, "weight*(isMC==0  && category =="+c+" && bdt < "+BDTb+" && bdt >= "+BDTc+veto+")");
        ...
        ttree->Draw("tripletMass>>hSignal3"+binning, "weight*(isMC>0  && category =="+c+" && bdt < "+BDTb+" && bdt >= "+BDTc+veto+")");
