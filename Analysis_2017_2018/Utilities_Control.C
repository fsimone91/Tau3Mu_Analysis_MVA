...
void ntupleClass_Control::FillHistoAC(Int_t ind, Int_t mu[NTOT], TH1F *hMinSegmComp, TH1F *hChi2Track, TH1D *hNMatchedStat, TH1D *hFlightDist, TH1D *hFlightDist_Signif, TH2D *hFlightDistvsP, TH1D *hPtErrOverPt, TH1D *hmassdi, Double_t dimu[NTOT], TH1F *hmassQuad, TH1F *hmassQuad_Zero, TH1D *hChi2VertexNorm, TH1D *hSegmComp, TH1D *hDeltaR, TH1D *hTrIPSign){
    ...
    hFlightDist->Fill(FlightDistPVSV2->at(ind)), pileupFactor;
    hFlightDist_Signif->Fill(FlightDistPVSV2_Significance->at(ind), pileupFactor);
...
void ntupleClass_Control::FillHistoSingleMu(Int_t mu_Ind[NTOT], Int_t mu[NTOT], TH1D *hist_pt, TH1D *hist_pt_mu[NMU_C], TH1D *hist_eta, TH1D *hist_eta_mu[NMU_C], TH1D *hist_phi, TH1D *hVx, TH1D *hVy, TH1D *hVz, TH1D *hPt_Tr, TH1D *hEta_Tr){
    ...
    for(int i=0; i<NMU_C; i++){
        ...
        hist_eta->Fill(abs(eta[i]), pileupFactor);
        hist_eta_mu[i]->Fill(abs(eta[i]), pileupFactor);
    ...
    hVx->Fill(Track_vx->at(mu[2]), pileupFactor);
    hVy->Fill(Track_vy->at(mu[2]), pileupFactor);
    hVz->Fill(Track_vz->at(mu[2]), pileupFactor);
    hPt_Tr->Fill(Track_pt->at(mu[2]), pileupFactor);
...
void ntupleClass_Control::FillHistoTriplet(Int_t ind, TH1D *hist_pt, TH1D *hist_eta, TH1D *hist_phi, TH1D *hist_mass){
    ...
    hist_phi->Fill(Triplet2_Phi->at(ind), pileupFactor);
    hist_mass->Fill(Triplet2_Mass->at(ind), pileupFactor);
}
...
void ntupleClass_Control::StudyOnTriplet(TString type, Int_t ind, Int_t mu[NTOT], TH1D *hDeltaX, TH1D *hDeltaY, TH1D *hDeltaZ, TH1D *hPt_tripl){
    ...
    hDeltaX->Fill(abs(Muon_vx->at(mu[0]) - Track_vx->at(mu[2])), pileupFactor );
    hDeltaX->Fill(abs(Muon_vx->at(mu[1]) - Track_vx->at(mu[2])), pileupFactor );
    hDeltaY->Fill(abs(Muon_vy->at(mu[0]) - Muon_vy->at(mu[1])) , pileupFactor );
    hDeltaY->Fill(abs(Muon_vy->at(mu[0]) - Track_vy->at(mu[2])), pileupFactor );
    hDeltaY->Fill(abs(Muon_vy->at(mu[1]) - Track_vy->at(mu[2])), pileupFactor );
    hDeltaZ->Fill(abs(Muon_vz->at(mu[0]) - Muon_vz->at(mu[1])) , pileupFactor );
    hDeltaZ->Fill(abs(Muon_vz->at(mu[0]) - Track_vz->at(mu[2])), pileupFactor );
    hDeltaZ->Fill(abs(Muon_vz->at(mu[1]) - Track_vz->at(mu[2])), pileupFactor );
    hPt_tripl->Fill(Triplet2_Pt->at(ind), pileupFactor);
}
...
void ntupleClass_Control::TriggerRequirements(Int_t ind, TH1D *hTripTriggerMatched){
    ...
    // HLT Trigger matching w/ "HLT_DoubleTr_Trk_Tau3mu_v*"
    for(int h=0; h<Trigger_hltname->size(); h++) {
        hltName = Trigger_hltname->at(h);
        if(strncmp(hltName, "HLT_DoubleTr_Trk_Tau3mu_v", 26) == 0) {
            nMatches++;
            hlt_ind = h;
        }
    }
...
void ntupleClass_Control::TreeFin_Fill(TTree *tree, Int_t ind, Int_t mu_Ind[NTOT], Int_t mu[NTOT], Double_t &run, Double_t &lumi, Double_t &evt, Double_t &puFactor, Double_t &Pmu3, Double_t &cLP, Double_t &tKink, Double_t &segmComp, Double_t &tripletMass, Double_t &tripletMassReso, Double_t &fv_nC, Double_t &fv_dphi3D, Double_t &fv_d3D, Double_t &fv_d3Dsig, Double_t &bs_sv_d2Dsig, Double_t &bs_sv_d2D, Double_t &pv_sv_dxy_sig, Double_t &pv_sv_dxy, Double_t& pv_sv_dxy_err, Double_t& pv_sv_dxy_smeared, Double_t& pv_sv_dxy_sig_corrected, Double_t& smearing, Double_t& dxy_shift, Double_t &d0, Double_t &d0sig, Double_t &mindca_iso, Double_t &trkRel, Double_t &Pmu1, Double_t &Ptmu1, Double_t &etamu1, Double_t &Pmu2, Double_t &Ptmu2, Double_t &etamu2, Double_t &Ptmu3, Double_t &etamu3, Double_t &P_trip, Double_t &Pt_trip, Double_t &eta_trip,  Double_t &phiMass, Double_t &nStationsMu1, Double_t &nStationsMu2, Double_t &nStationsMu3, Double_t &Iso03Mu1, Double_t &Iso03Mu2, Double_t &Iso03Mu3, Double_t &Iso05Mu1, Double_t &Iso05Mu2, Double_t &Iso05Mu3, Double_t &nMatchesMu1, Double_t &nMatchesMu2, Double_t &nMatchesMu3, Double_t &timeAtIpInOutMu1, Double_t &timeAtIpInOutMu2, Double_t &timeAtIpInOutMu3, Double_t &cQ_uS, Double_t &cQ_tK, Double_t &cQ_gK, Double_t &cQ_tRChi2, Double_t &cQ_sRChi2, Double_t &cQ_Chi2LM, Double_t &cQ_Chi2lD, Double_t &cQ_gDEP, Double_t &cQ_tM, Double_t &cQ_gTP, Double_t &calEn_emMu1, Double_t &calEn_emMu2, Double_t &calEn_emMu3, Double_t &calEn_hadMu1, Double_t &calEn_hadMu2, Double_t &calEn_hadMu3, Double_t &caloComp, Double_t &fliDistPVSV_Chi2, Double_t &isGlb3, Double_t &isTracker3, Double_t &isLoose3, Double_t &isSoft3, Double_t &isPF3, Double_t &isRPC3, Double_t &isSA3, Double_t &isCalo3, Double_t &Vx1, Double_t &Vx2, Double_t &Vx3, Double_t &Vy1, Double_t &Vy2, Double_t &Vy3, Double_t &Vz1, Double_t &Vz2, Double_t &Vz3, Double_t &RefVx1, Double_t &RefVx2, Double_t &RefVx3, Double_t &RefVy1, Double_t &RefVy2, Double_t &RefVy3, Double_t &RefVz1, Double_t &RefVz2, Double_t &RefVz3, Double_t &SVx, Double_t &SVy, Double_t &SVz, Double_t &had03, Double_t &had05, Double_t &nJets03, Double_t &nJets05, Double_t &nTracks03, Double_t &nTracks05, Double_t &sumPt03, Double_t &sumPt05, Double_t &hadVeto03, Double_t &hadVeto05, Double_t &emVeto03, Double_t &emVeto05, Double_t &trVeto03, Double_t &trVeto05){
    ...
    if (FlightDistPVSV2_Err->at(ind) == 0 || isnan(FlightDistPVSV2_Significance->at(ind)) || isnan(FlightDistBS_SV_Significance->at(ind)) || isnan(DistXY_significance_PVSV->at(ind))) {
        fv_d3Dsig = -999.; 
        bs_sv_d2Dsig = -999.;
        pv_sv_dxy_sig = -999.;
        pv_sv_dxy_err = -999.;
        cout<<"Warning: isnan(FlightDistPVSV2_Significance->at(ind))\n evt lumi run "<<evt<<" "<<lumi<<" "<<run<<endl;
        cout<<" FlightDistPVSV2_Significance->at(ind) "<<FlightDistPVSV2_Significance->at(ind)<<endl;
        cout<<" FlightDistPVSV2_Err->at(ind) "<<FlightDistPVSV2_Err->at(ind)<<endl;
        cout<<" FlightDistPVSV2->at(ind) "<<FlightDistPVSV2->at(ind)<<endl;
        cout<<" FlightDistBS_SV_Significance->at(ind) "<<FlightDistBS_SV_Significance->at(ind)<<endl;
        cout<<" DistXY_significance_PVSV->at(ind) "<<DistXY_significance_PVSV->at(ind)<<endl;
        cout<<" TripletVtx2_Chi2->at(ind) "<<TripletVtx2_Chi2->at(ind)<<endl;
    }else{ 
    ...
    Iso03Mu1 = Muon_nTracks03->at(mu[0]);
    Iso03Mu2 = Muon_nTracks03->at(mu[1]);
    Iso03Mu3 = 999;
    ...
    eta_trip = Triplet2_Eta->at(ind);
        //
    //Phi Mass
    phiMass = DimuonMass(MuonCharge->at(mu[0]), MuonCharge->at(mu[1]), MuonPt->at(mu[0]), MuonPt->at(mu[1]), MuonEta->at(mu[0]), MuonEta->at(mu[1]), MuonPhi->at(mu[0]), MuonPhi->at(mu[1])); // dimuon mass 1-2
    //
    ...
    isLoose3 =   Muon_isLoose->at(mu[1]);
    isSoft3 =    Muon_isSoft->at(mu[1]);
    isPF3 =      Muon_isPF->at(mu[1]);
    isRPC3 =     Muon_isRPCMuon->at(mu[1]);
    isSA3 =      Muon_isStandAloneMuon->at(mu[1]);
    isCalo3 =    Muon_isCaloMuon->at(mu[1]);
    //
    Vx1 = Muon_vx->at(mu[0]);
    Vx2 = Muon_vx->at(mu[1]);
    ...
    had03 = Muon_hadEt03->at(mu[1]);
    had05 = Muon_hadEt05->at(mu[1]);
    nJets03 = Muon_nJets03->at(mu[1]);
    nJets05 = Muon_nJets05->at(mu[1]);
    nTracks03 = Muon_nTracks03->at(mu[1]);
    nTracks05 = Muon_nTracks05->at(mu[1]);
    sumPt03 = Muon_sumPt03->at(mu[1]);
    sumPt05 = Muon_sumPt05->at(mu[1]);
    hadVeto03 = Muon_hadVetoEt03->at(mu[1]);
    hadVeto05 = Muon_hadVetoEt05->at(mu[1]);
    emVeto03 = Muon_emVetoEt03->at(mu[1]);
    emVeto05 = Muon_emVetoEt05->at(mu[1]);
    trVeto03 = Muon_trackerVetoPt03->at(mu[1]);
    trVeto05 = Muon_trackerVetoPt05->at(mu[1]);
...
void ntupleClass_Control::TreeFin_Init(TTree *&tree, Double_t &run, Double_t &lumi, Double_t &evt, Double_t &puFactor, Double_t &Pmu3, Double_t &cLP, Double_t &tKink, Double_t &segmComp, Double_t &tripletMass, Double_t &tripletMassReso, Double_t &fv_nC, Double_t &fv_dphi3D, Double_t &fv_d3D, Double_t &fv_d3Dsig, Double_t &bs_sv_d2Dsig, Double_t &bs_sv_d2D, Double_t &pv_sv_dxy_sig, Double_t &pv_sv_dxy, Double_t& pv_sv_dxy_err, Double_t& pv_sv_dxy_smeared, Double_t& pv_sv_dxy_sig_corrected, Double_t& smearing, Double_t& dxy_shift, Double_t &d0, Double_t &d0sig, Double_t &mindca_iso, Double_t &trkRel, Double_t &Pmu1, Double_t &Ptmu1, Double_t &etamu1, Double_t &Pmu2, Double_t &Ptmu2, Double_t &etamu2, Double_t &Ptmu3, Double_t &etamu3, Double_t &P_trip, Double_t &Pt_trip, Double_t &eta_trip, Double_t &phiMass, Double_t &nStationsMu1, Double_t &nStationsMu2, Double_t &nStationsMu3, Double_t &Iso03Mu1, Double_t &Iso03Mu2, Double_t &Iso03Mu3, Double_t &Iso05Mu1, Double_t &Iso05Mu2, Double_t &Iso05Mu3, Double_t &nMatchesMu1, Double_t &nMatchesMu2, Double_t &nMatchesMu3, Double_t &timeAtIpInOutMu1, Double_t &timeAtIpInOutMu2, Double_t &timeAtIpInOutMu3, Double_t &cQ_uS, Double_t &cQ_tK, Double_t &cQ_gK, Double_t &cQ_tRChi2, Double_t &cQ_sRChi2, Double_t &cQ_Chi2LM, Double_t &cQ_Chi2lD, Double_t &cQ_gDEP, Double_t &cQ_tM, Double_t &cQ_gTP, Double_t &calEn_emMu1, Double_t &calEn_emMu2, Double_t &calEn_emMu3, Double_t &calEn_hadMu1, Double_t &calEn_hadMu2, Double_t &calEn_hadMu3, Double_t &caloComp, Double_t &fliDistPVSV_Chi2, Double_t &isGlb3, Double_t &isTracker3, Double_t &isLoose3, Double_t &isSoft3, Double_t &isPF3, Double_t &isRPC3, Double_t &isSA3, Double_t &isCalo3, Double_t &Vx1, Double_t &Vx2, Double_t &Vx3, Double_t &Vy1, Double_t &Vy2, Double_t &Vy3, Double_t &Vz1, Double_t &Vz2, Double_t &Vz3, Double_t &RefVx1, Double_t &RefVx2, Double_t &RefVx3, Double_t &RefVy1, Double_t &RefVy2, Double_t &RefVy3, Double_t &RefVz1, Double_t &RefVz2, Double_t &RefVz3, Double_t &SVx, Double_t &SVy, Double_t &SVz, Double_t &had03, Double_t &had05, Double_t &nJets03, Double_t &nJets05, Double_t &nTracks03, Double_t &nTracks05, Double_t &sumPt03, Double_t &sumPt05, Double_t &hadVeto03, Double_t &hadVeto05, Double_t &emVeto03, Double_t &emVeto05, Double_t &trVeto03, Double_t &trVeto05){
    // Set tree branches
